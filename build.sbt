...
lazy val root = (project in file("."))
  .settings(
    gen := { val exitCode = Process(Seq("java","-cp", antlr_jar, "org.antlr.v4.Tool", "-o",((sourceManaged in Compile).value).getPath + extra, "-no-listener", "-visitor", "src/main/mc/parser/MC.g4")).!
    			if(exitCode != 0) sys.error(s"Antlr4 failed with exit code $exitCode") 
    			exitCode
    		  },
    grun := {
    	val exitCode = Process(Seq("java","-cp",antlr_jar, "org.antlr.v4.gui.TestRig")).!
    	if(exitCode != 0) sys.error(s"Antlr4 failed with exit code $exitCode")
    }
 )
